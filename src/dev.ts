  ...
  const watcher = watch(config.root, { recursive: true }, (_event, filename) => {
    ...
    timer = setTimeout(async () => {
      try {
        ...
        } else if (/^(page|actions|error|not-found)\.[jt]sx?$/.test(name)) send("boundary", "page");
        else send("reload");
