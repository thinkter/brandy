...
4. **Progressive enhancement by construction**, not as an afterthought: no-JS path is the full render; JS path is the partial swap. *Scoped exception (decided 2026-07-07, issue #6):* on a streamed cold load (`loading.tsx`), deferred content ships as inert declarative templates applied by Brandy's always-shipped runtime — without JS the skeleton remains until reload. This is the one place the guarantee is relaxed, and deliberately so: streamed responses carry no inline scripts (CSP-safe), and every route without `loading.tsx` keeps the full guarantee.
...
- **Streaming and the no-JS guarantee (decided — issue #6).** Deferred `loading.tsx` content, head metadata, and mid-stream redirects arrive as inert declarative `<template>` elements applied by the always-shipped client runtime (a boot sweep on cold loads; the existing reconciliation on soft navigations). Streamed responses carry **no inline scripts**, so they work under a strict `Content-Security-Policy` with no nonce plumbing. The documented, scoped exception to principle 4: a no-JS cold load of a streaming route shows the skeleton until reload. Routes without `loading.tsx` are unaffected.
